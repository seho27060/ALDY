...
import { getReviewList, getRequestedCode, getRequestingCode, getEditedCode, getFinalCode } from '../../api/code'
    ...
    <main>
      ...
      <section className='request-list'>
        <div>
          ...
          className={`tabBtn ${tab === 'finalCode' ? 'active': ""}`}>최종 제출 코드</button>
        </div>
        <div>
        ...
        {
          tab === 'finalCode' && <FinalCode setFinalCodeModalShow={setFinalCodeModalShow}/>
        }
        </div>
      </section>
    </main>
...
const FinalCode = (props) => {
  ...
  return (
    <div>
      {
        list?.map((item) => <CardFinalCode item={item} setFinalCodeModalShow={setFinalCodeModalShow}/>)
      }
      <Paging
        page={pageNum}
        setPage={setPageNum}
    ...
    <Container className='review-list-item'>
      <Row>
        ...
        <Col><button className='correctBtn' onClick={()=>{
          ...
          sessionStorage.setItem('reviewStudyName', item.codeDto.studyDto.name)
          sessionStorage.setItem('reviewProblemNum', item.codeDto.problemNum)
          sessionStorage.setItem('reviewProblemName', item.codeDto.problemName)
          navigate('/review')
    ...
    <Modal.Body>
      <div>
        <div className='final-code-header'>
          <div style={{'paddingRight':'20px'}}>✨스터디 이름:&nbsp; {finalStudyName}</div>
          <div style={{'paddingLeft':'20px'}}>문제번호:&nbsp; {finalProblemNum}✨</div>
        </div>
